...
function removeFirstOccurrences(str, value) {
    let valuepos = str.indexOf(value);
    return str.substr(0, valuepos) + str.substr(valuepos+value.length);
}
...
function getRectangleString(width, height) {
    let uppart = '┌' + '─'.repeat(width-2) + '┐' + '\n';
    let middlepart = '│' + ' '.repeat(width-2) + '│' + '\n';
    middlepart = middlepart.repeat(height-2);
    let lowpart = '└' + '─'.repeat(width-2) + '┘' + '\n';
    return uppart + middlepart + lowpart;
}
...
function encodeToRot13(str) {
    ...
    for (let i=0;i<str.length;i++)
    {
        ...
        {
            res +=String.fromCharCode((str.charCodeAt(i) - 65 + 13) % 26 + 65);
        }
        else
        if (str.charAt(i)>='a' && str.charAt(i)<='z')
        {
            res +=String.fromCharCode((str.charCodeAt(i) - 97 + 13) % 26 + 97);
        }
        else
            res += str.charAt(i);
    }
